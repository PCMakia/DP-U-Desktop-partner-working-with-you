<script lang="ts">
	...
	let nextBlinkTime = $state(workshopBlinkInterval());
	...
	function normalizeModel(loadedVrm: VRM) {
		...
		sceneBaseYaw = config.sceneRotationY;
	...
	function holdWorkshopIdle(action: THREE.AnimationAction) {
		action.paused = true;
		action.timeScale = 0;
	...
	function playIdleLoop(action: THREE.AnimationAction, fade = 0.3) {
	...
	function scheduleIdleCycle(targetVrm: VRM, targetMixer: THREE.AnimationMixer, duration: number) {
		...
		if (workshopStore.active) return;
		...
		idleCycleTimeout = setTimeout(() => {
			if (!shouldTalk && !isEmotePlaying && !photomodeStore.active && !workshopStore.active) {
	...
	$effect(() => {
		...
		if (speaking && currentTalkingClip) {
			...
			if (currentIdleAction) {
				playIdleLoop(currentIdleAction);
			}
					...
					const onFinished = (e: { action: THREE.AnimationAction }) => {
						if (e.action === action) {
							...
							if (capturedIdleAction) {
								playIdleLoop(capturedIdleAction);
							}
	...
	$effect(() => {
		const nonce = workshopStore.blinkNonce;
		if (!nonce) return;
		untrack(() => {
			isBlinking = true;
			blinkProgress = 0;
	...
	$effect(() => {
		const quiet = workshopStore.active;
		untrack(() => {
			const targetVrm = vrm;
			const targetMixer = mixer;
			if (!targetVrm || !targetMixer || photomodeStore.active) return;
			if (idleCycleTimeout) {
				clearTimeout(idleCycleTimeout);
	...
	const workshopLookPoint = new THREE.Vector3();
	const lookAtAnchor = new THREE.Object3D();
	let sceneBaseYaw = 0;
	const workshopNeckDelta = new THREE.Quaternion();
	...
	useTask((delta) => {
		...
		if (vrm && workshopHeadHasDelta) {
			const head = vrm.humanoid.getNormalizedBoneNode('head');
			const neck = vrm.humanoid.getNormalizedBoneNode('neck');
			if (neck) neck.quaternion.multiply(workshopUndoQuat.copy(workshopNeckDelta).invert());
		...
		// Workshop: neck/head take ±30° first; body yaw is applied on the root.
		// Do not slerp toward a world look point here — that locked yaw against
		// the rotating body. Photo-mode still tracks the scene camera.
		if (!workshopLook && vrm.lookAt) {
			vrm.lookAt.target = null;
		}
		if (workshopLook) {
			const head = vrm.humanoid.getNormalizedBoneNode('head');
			const neck = vrm.humanoid.getNormalizedBoneNode('neck');
			const yawSign = (vrm.meta?.metaVersion === '1' ? 1 : -1) * CURSOR_YAW_SIGN;
			const yaw = THREE.MathUtils.clamp(workshopStore.headYaw, -NECK_YAW_LIMIT, NECK_YAW_LIMIT);
			const pitch = THREE.MathUtils.clamp(workshopStore.headPitch, -0.28, 0.22);
			camera.current.getWorldQuaternion(parentQuat);
			camRight.set(1, 0, 0).applyQuaternion(parentQuat);
			const applyLook = (
				...
				const addAxis = (axisWorld: THREE.Vector3, angle: number) => {
					if (Math.abs(angle) < 1e-8) return;
					bone.parent!.updateWorldMatrix(true, false);
					bone.parent!.getWorldQuaternion(parentQuat);
					lookDir.copy(axisWorld).applyQuaternion(workshopUndoQuat.copy(parentQuat).invert());
					if (lookDir.lengthSq() < 1e-10) return;
					lookDir.normalize();
					lookQuat.setFromAxisAngle(lookDir, angle);
					acc.multiply(lookQuat);
					bone.quaternion.multiply(lookQuat);
				};
				addAxis(worldUp, yawAmt * yawSign);
				addAxis(camRight, pitchAmt * CURSOR_PITCH_SIGN);
			};
			applyLook(neck, yaw * 0.35, pitch * 0.3, workshopNeckDelta);
			applyLook(head, yaw * 0.65, pitch * 0.7, workshopHeadDelta);
			workshopHeadHasDelta = true;
		...
		if (!workshopLook && headTrackWeight > 0.001 && camera.current) {
			const head = vrm.humanoid.getNormalizedBoneNode('head');
			if (head?.parent) {
		...
		if (!isEmotePlaying) {
			...
			if (isBlinking) {
				...
				if (blinkProgress >= 1) {
					...
					isBlinking = false;
					blinkTimer = 0;
					nextBlinkTime = workshopStore.active ? workshopBlinkInterval() : Math.random() * 4 + 2;
