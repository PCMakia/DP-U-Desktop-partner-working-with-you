<script lang="ts">
	...
	let { centered = false, locked = false, overlay = false, companion = false }: Props = $props();
	...
	const workshopOn = $derived((overlay || companion) && workshopStore.enabled && !photomodeStore.active);

	onMount(() => {
		if (overlay || companion) workshopStore.attachCompanion();
		const checkDarkMode = () => {
		...
		return () => {
			if (overlay || companion) workshopStore.detachCompanion();
			observer.disconnect();
			unregisterCapture();
	...
	function computeFit(vrm: VRM): { center: number; halfSpan: number } {
		...
		// Workshop: waist-up 3/4 so a standing rest clip reads seated.
		if (workshopOn) {
			let chestY = headTop * 0.62;
			const chest =
				vrm.humanoid?.getNormalizedBoneNode('chest') ??
				vrm.humanoid?.getNormalizedBoneNode('spine');
			if (chest) {
				const p = new Vector3();
				chest.getWorldPosition(p);
				chestY = p.y;
			}
			const top = headTop + (headTop - chestY) * 0.08;
			return { center: (top + chestY) / 2, halfSpan: (top - chestY) / 2 };
		}
	...
	function applyCamera() {
		...
		if (workshopOn) {
			// 3/4, slightly above seated eyeline, looking toward the monitor
			const yaw = 0.62;
			cam.position.set(Math.sin(yaw) * distance, targetY + 0.1, Math.cos(yaw) * distance);
			cam.lookAt(-0.16, targetY - 0.04, 0);
			if (controls) {
				controls.target.set(-0.16, targetY - 0.04, 0);
				controls.update();
			}
		} else {
...
{/if}
{#if !overlay && !$isPresenting && !workshopOn && !(photomodeStore.active && photomodeStore.background.type !== 'room')}
...
{#if workshopOn}
	<T.AmbientLight intensity={0.22} color="#3a2418" />
	<T.DirectionalLight intensity={1.55} position={[0.45, 1.35, 0.55]} color="#ffc4a0" />
{:else}
