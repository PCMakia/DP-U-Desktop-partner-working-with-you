<script lang="ts">
	...
	async function launchOverlay() {
		...
		const popup = window.open(
			overlayUrl,
			'utsuwa-overlay',
			'popup=yes,width=420,height=720,left=80,top=80'
		);
		if (!popup) {
			window.location.assign(overlayUrl);
		}
	}
</script>
...
<div class="top-right-buttons" bind:this={rootEl}>
	<div class="button-row">
		...
		{/if}
		{#if showOverlayBtn}
			<button class="icon-btn overlay-btn" onclick={launchOverlay} aria-label="Launch overlay" title="Open desk overlay (browser popup, or desktop pin in the Tauri app)">
