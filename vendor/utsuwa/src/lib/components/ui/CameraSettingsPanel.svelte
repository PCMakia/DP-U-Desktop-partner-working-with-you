...
<div class="camera-panel" role="dialog" aria-label="Camera settings">
	...
	{#if profile === 'overlay' || profile === 'main'}
		...
		<label class="toggle-row">
			<input
				type="checkbox"
				checked={workshopStore.enabled}
				onchange={(e) => workshopStore.setEnabled(e.currentTarget.checked)}
			/>
			<span>Sitting bench (rest pose, 3/4 camera)</span>
		</label>
		<label class="toggle-row">
			<input
				type="checkbox"
				checked={workshopStore.lookAt === 'cursor'}
				onchange={(e) => workshopStore.setLookAt(e.currentTarget.checked ? 'cursor' : 'editor')}
			/>
			<span>Look-at follows cursor</span>
		</label>
		<p class="hint">
			Ctrl+Alt+Right Shift hides or shows overlay buttons even when another app is focused. Only
			that combo is claimed; other apps keep their own shortcuts.
		</p>
	{/if}
