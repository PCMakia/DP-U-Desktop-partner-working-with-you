...
class UtsuwaDatabase extends Dexie {
	...
	constructor() {
		...
		this.version(7)
			.stores({
				characterStates: '++id, updatedAt',
				facts: '++id, memoryKey, category, importance, createdAt',
				sessions: '++id, memoryKey, startedAt',
				conversationTurns: '++id, memoryKey, sessionId, createdAt',
				completedEvents: '++id, eventId, completedAt',
				reminders: '++id, sessionId, triggerAt, executed, dismissed, [executed+triggerAt]'
			})
			.upgrade(async (tx) => {
				const owner = 'person/owner';
				await tx.table('facts').toCollection().modify((row: { memoryKey?: string }) => {
					if (!row.memoryKey) row.memoryKey = owner;
				});
				await tx.table('sessions').toCollection().modify((row: { memoryKey?: string }) => {
					if (!row.memoryKey) row.memoryKey = owner;
				});
				await tx.table('conversationTurns').toCollection().modify((row: { memoryKey?: string }) => {
					if (!row.memoryKey) row.memoryKey = owner;
				});
			});
	}
