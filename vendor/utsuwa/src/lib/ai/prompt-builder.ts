...
import { OWNER_PERSON_KEY, YUE_VOICE_LOCK } from '../alice-yue/memory/keys.ts';
...
export interface PromptContext {
	...
	/** Who is talking this turn. UI is always the owner (User). */
	speaker?: {
		memoryKey: string;
		isOwner: boolean;
		displayName: string;
...
function buildSpeakerLayer(ctx: PromptContext): string {
	const speaker = ctx.speaker ?? {
		memoryKey: OWNER_PERSON_KEY,
		isOwner: true,
		displayName: 'User',
		recognized: true,
		turnCount: ctx.state.totalInteractions
	};
	if (speaker.isOwner) {
		return `<speaker>
Memory key: ${speaker.memoryKey}
The speaker is User, your bound partner. You may use owner memories only.
Memory is the past. Do not write expected future scenes.
</speaker>`;
	}
	const known = speaker.recognized
		? `You have spoken with them about ${speaker.turnCount ?? 0} times. You may use only THIS stranger's memories.`
		: `You do not know them yet (${speaker.turnCount ?? 0} turns). No pet names, no User memories.`;
	return `<speaker>
Memory key: ${speaker.memoryKey}
The speaker is NOT User. Display: ${speaker.displayName}.
${known}
Do not greet them as your partner. Do not load person/owner memories.
Spoken words only. Do not write *asterisk* stage directions.
Do not simulate later scenes. The present is this message only.
</speaker>`;
}

function finalizePrompt(body: string, ctx: PromptContext): string {
	return [body, buildSpeakerLayer(ctx), `<voice_lock>\n${YUE_VOICE_LOCK}\n</voice_lock>`].join('\n\n');
}
...
export function buildSystemPrompt(context: PromptContext): string {
	...
	if (context.state.appMode === 'companion') {
		return finalizePrompt(buildCompanionModePrompt(context), context);
	}
	const layers = [
	...
	return finalizePrompt(layers.join('\n\n'), context);
}
...
RULES:
- Be in character. Do not pad replies to sound helpful or engaging.
- Keep responses as short as the character's speech rules require.
- Remember context from recent conversations with THIS speaker only
...
Energy: ${energyDesc} (${ctx.state.energy}/100)
	...
	if (mem.relevantFacts.length > 0) {
		...
		const speakerKey = ctx.speaker?.memoryKey;
		const scoped = mem.relevantFacts.filter(
			(f) => !f.memoryKey || !speakerKey || f.memoryKey === speakerKey
		);
		const factsText = scoped.slice(0, factLimit).map((f) => `- ${f.content}`).join('\n');
		if (factsText) memorySections.push(`Things you know about them:\n${factsText}`);
	}
...
Respond naturally as ${ctx.persona.name}. Stay in her voice. Do not pad.
...
- Keep responses conversational and natural. If the character is terse, stay terse. Do not pad to multiple paragraphs.
...
function buildMemoryLayer(ctx: PromptContext): string {
	...
	if (mem.relevantFacts.length > 0) {
		...
		const factsText = scoped.slice(0, factLimit).map((f) => `- ${f.content}`).join('\n');
		if (factsText) sections.push(`Things you know about them:\n${factsText}`);
	}
