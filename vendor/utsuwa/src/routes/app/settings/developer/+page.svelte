<script lang="ts">
	...
	import {
		controlAliceYueBrain,
		getAliceYueBrainStatus
	} from '$lib/alice-yue/brain';
	...
	let brainNote = $state('');
	let llamaUp = $state<boolean | null>(null);

	async function refreshBrainStatus() {
		try {
			const status = await getAliceYueBrainStatus();
			llamaUp = status.llama;
			brainNote = status.llama ? 'Eclipse is up on 127.0.0.1:8081' : 'Eclipse is parked (llama.cpp is down)';
		} catch (e) {
			llamaUp = null;
			brainNote = e instanceof Error ? e.message : 'Could not read brain status';
		}
	}

	async function runBrain(action: 'wake' | 'park', discord = false) {
		brainBusy = true;
		brainNote = action === 'park' ? 'Parking…' : discord ? 'Starting Eclipse + Discord…' : 'Starting Eclipse…';
		try {
			await controlAliceYueBrain(action, { discord });
			if (action === 'wake') {
				for (let i = 0; i < 90; i++) {
					await new Promise((r) => setTimeout(r, 2000));
					const status = await getAliceYueBrainStatus().catch(() => ({ llama: false }));
					if (status.llama) {
						llamaUp = true;
						brainNote = discord
							? 'Eclipse is up. Discord window should be open too.'
							: 'Eclipse is up. Chat and PTT can send.';
						return;
					}
				}
				brainNote = 'Started the wake script, but Eclipse has not answered yet. Watch the llama.cpp window.';
			} else {
				await refreshBrainStatus();
			}
		} catch (e) {
			brainNote = e instanceof Error ? e.message : 'Brain control failed';
		} finally {
			brainBusy = false;
		}
	}
		...
		<div class="controls-panel">
			<section class="section">
				...
				<div class="quick-actions">
					<button class="action-btn" onclick={() => runBrain('wake')} disabled={brainBusy}>
						Start Eclipse
					</button>
					<button class="action-btn" onclick={() => runBrain('wake', true)} disabled={brainBusy}>
						Start Eclipse + Discord
					</button>
					<button class="action-btn" onclick={() => runBrain('park')} disabled={brainBusy}>
						Park brain
					</button>
					<button class="action-btn" onclick={() => refreshBrainStatus()} disabled={brainBusy}>
						Refresh
					</button>
				</div>
			</section>
