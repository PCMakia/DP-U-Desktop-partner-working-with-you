<script lang="ts">
	...
	import OverlayClickThrough from '$lib/components/overlay/OverlayClickThrough.svelte';
	import OverlayPresence from '$lib/components/overlay/OverlayPresence.svelte';
	...
	import { isChromeToggleHotkey } from '$lib/stores/workshop-logic';
	...
	// Workshop overlay does not retrieve memories until you open chat. Skip
	// Transformers.js here so the sit loop is only the VRM viewer.
	$effect(() => {
		if (workshopStore.enabled) return;
	...
	// main app window is hidden. Workshop quiet mode never sends those through
	// the LLM — only an explicit chat/PTT hotkey talks to Eclipse.
	$effect(() => {
		if (workshopStore.enabled) {
	...
	// Move the overlay without native startDragging so Windows snap layouts
	// do not steal the window to half/quarter screen.
	let moving = $state(false);
	let moveStart = { x: 0, y: 0, wx: 0, wy: 0 };
	let moveRaf = 0;
	let lastMove = { x: 0, y: 0 };

	async function onMoveStart(e: PointerEvent) {
		if (!isTauri() || positionLocked) return;
		e.preventDefault();
		e.stopPropagation();
		(e.currentTarget as HTMLElement).setPointerCapture(e.pointerId);
		try {
			...
			console.error('Failed to start overlay move:', err);
		}
	}
	...
	function onMoveMove(e: PointerEvent) {
		if (!moving) return;
		lastMove = {
			x: moveStart.wx + (e.screenX - moveStart.x),
			y: moveStart.wy + (e.screenY - moveStart.y)
		};
		if (moveRaf) return;
		moveRaf = requestAnimationFrame(async () => {
			moveRaf = 0;
			try {
				const { getCurrentWindow, LogicalPosition } = await import('@tauri-apps/api/window');
				await getCurrentWindow().setPosition(new LogicalPosition(lastMove.x, lastMove.y));
			} catch (err) {
				console.error('Failed to move overlay:', err);
			}
		});
	}
	...
	function stopRailPointer(e: PointerEvent) {
		e.stopPropagation();
	}

	function toggleOverlayChrome() {
		overlayStore.toggleChromeHidden();
		if (overlayStore.chromeHidden) {
			showCamera = false;
		}
	}
	...
	$effect(() => {
		if (overlayStore.chromeHidden) showCamera = false;
	});
</script>
...
<div
	...
	class:workshop={workshopStore.enabled}
	class:chrome-hidden={overlayStore.chromeHidden}
>
	<OverlayClickThrough cameraOpen={showCamera} resizing={resizing || moving} />
	<OverlayPresence />

	<!-- VRM Scene (fills the overlay) - locked to prevent rotation when dragging -->
	<!-- svelte-ignore a11y_no_static_element_interactions -->
	<div class="scene-container">
		<VrmScene overlay={true} locked={true} companion />
	</div>

	<!-- Hover chrome: soft frame + corner resize tab -->
	<div class="overlay-frame"></div>
	{#if isTauri() && !overlayStore.chromeHidden}
		<div
			class="resize-tab"
	...
	{#if !overlayStore.chromeHidden}
	<!-- Control rail: grip moves the window; buttons stay clickable -->
	<div class="overlay-rail bench-strip">
		<div
			class="rail-grip"
			...
			title="Drag to move (does not snap)"
		></div>
		<button
			class="rail-btn"
			type="button"
		...
		<button
			class="rail-btn"
			type="button"
			class:rail-btn-active={workshopStore.enabled}
			onpointerdown={stopRailPointer}
			onclick={() => workshopStore.setEnabled(!workshopStore.enabled)}
			aria-label={workshopStore.enabled ? 'Exit workshop bench' : 'Enter workshop bench'}
			title={workshopStore.enabled ? 'Workshop on — Ctrl+Alt+Right Shift hides chrome' : 'Workshop off'}
		>
	...
	{#if overlayStore.chatExpanded || latestResponse || isTyping}
		<SpeechBubble
	...
	{#if !workshopStore.enabled || overlayStore.chatExpanded}
...
<style>
	...
	.rail-grip {
		...
		opacity: 0.55;
		background-image: radial-gradient(circle, var(--text-secondary) 1.2px, transparent 1.4px);
	...
	.workshop .overlay-frame {
		display: none;
	}

	.workshop .scene-container,
	.workshop .scene-container:active {
		cursor: default;
	}
	...
	.chrome-hidden .overlay-frame,
	.chrome-hidden .resize-tab,
	.chrome-hidden .overlay-rail {
		display: none;
	}
