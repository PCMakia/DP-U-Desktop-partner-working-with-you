# DP&U — Desktop partner working with you
...
| `rules`                                                                         | Hard constraints (voice length, stranger vs owner, 18+ no minors character). |
...
No OpenAI / Anthropic keys. Inference is local llama.cpp. Overlay scripts set `DPU_ROOT` to this repo so park/wake can find the PowerShell helpers.
...
# Chooser window, then Tauri desktop UI
.\DPU.bat
# or DPU.exe  (filename is DPU, not DP&U, because & is a cmd.exe separator)
```

The small window offers:

- **UI only (no llama)** — Tauri main + overlay. No Eclipse in VRAM.
- **UI and llama** — Tauri + `scripts\04-serve.ps1`.
- **Full** — Tauri + llama + Discord (needs `.env`).

Press **Start**. Consoles stay hidden. Closing the Tauri main window stops llama.cpp, Discord, and the UI (frees VRAM; it does not delete models). Overlay: monitor button or `Ctrl+Shift+U`.

If Utsuwa is missing, a small window says to run `.\scripts\00-fetch-utsuwa.ps1` at the repo root.

llama.cpp API: `http://127.0.0.1:8081/v1`

Browser `pnpm dev` (`scripts\07-utsuwa-ui.ps1`) is optional for UI debugging. Daily use is Tauri (`scripts\13-utsuwa-desktop.ps1`).
